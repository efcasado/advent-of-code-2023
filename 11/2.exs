defmodule AOC23.D11 do
  def run(input, m) do
    ...
    gs
    |> pairs
    |> Enum.reduce(0, fn({from, to}, acc) -> distance(from, to, xrows, xcols, m) + acc end)
  end
  ...
  def distance({_name1, {x1, y1}}, {_name2, {x2, y2}}, xrows, xcols, m) do
    (abs(x2 - x1) + abs(y2 - y1)) + (xings(xrows, x1, x2) * m) + (xings(xcols, y1, y2) * m)
  end
...
result    = AOC23.D11.run(input, 999_999)
